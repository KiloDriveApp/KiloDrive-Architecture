...
| Authoritative foreign exchange | Implemented source authority; operational activation is pair/purpose/country specific | Versioned maker-checker rates produce expiring customer quotes and immutable transaction snapshots; settings and web lookups are not runtime authority | Active reviewed pair, evidence/freshness, fee policy, quote/capture/refund/dispute/reconciliation matrix and operator rollback exercise |
| Modular-monolith runtime profiles | Implemented/incremental deployment option | Combined, API, realtime, country-worker, media and reporting compositions register only owned surfaces and fail closed on invalid bindings | Startup-composition tests, one-writer proof, per-profile readiness, staged rollout and rollback evidence |
...
| Authoritative driver workspaces and active-trip guard | Implemented/incremental | Typed parent/child destinations preserve driver task context, while riders with assigned/active trips are directed to the existing journey instead of a second request form | Deep-link/back/resume/process-death tests, role/account isolation, active-trip races, accessibility and physical-device navigation evidence |
| Public manuals and search discovery | Implemented/incremental | Responsive manual pages, table of contents, search, illustrations, canonical metadata and a focused sitemap expose help without making artifact files canonical competitors | HTTP 200/indexability/canonical/HEAD tests, sitemap validation, localized content review, responsive/accessibility browser evidence |
